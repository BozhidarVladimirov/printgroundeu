...
console.log('Testing product 11031:');
const sku = '11031';
const colorCodes = productColors.get(sku);
const colorNames = Array.from(colorCodes).map(code => colorCodeToName[code] || 'Multi').sort();
console.log('Expected colors:', colorNames);
...
console.log('Colors match found:', !!colorsMatch);

if (colorsMatch) {
    console.log('Match[0]:', JSON.stringify(colorsMatch[0]));
    console.log('Match[1]:', JSON.stringify(colorsMatch[1]));
    
    const oldColors = colorsMatch[1].trim();
    const newColors = colorNames.map(c => `"${c}"`).join(', ');
    console.log('Old colors:', JSON.stringify(oldColors));
    console.log('New colors:', JSON.stringify(newColors));
    console.log('Contains "Multi":', oldColors.includes('"Multi"'));
    
    const fullPattern = colorsMatch[0];
    const replacement = `"colors": [${newColors}]`;
    console.log('Pattern:', JSON.stringify(fullPattern));
    console.log('Replacement:', JSON.stringify(replacement));
    
    // Check if pattern exists
    const patternExists = productsContent.includes(fullPattern);
    console.log('Pattern exists in content:', patternExists);
    
    // Try replacement
    const newContent = productsContent.replace(fullPattern, replacement);
    console.log('Replacement worked:', newContent !== productsContent);
    
    // Check result
    const newSkuIndex = newContent.indexOf(searchStr);
    const newAfterSku = newContent.substring(newSkuIndex);
    const newColorsMatch = newAfterSku.match(/"colors":\s*\[([^\]]*)\]/);
    console.log('New colors:', newColorsMatch ? newColorsMatch[1].trim() : 'NOT FOUND');
}
